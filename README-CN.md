...
| **广告查询 (Ads Query)** | 对广告数据做自描述式分析查询,结果直接返回到对话中 | Helium 10 Ads |
...
| "给我看指定 profile、指定日期范围的广告花费 / ACoS / ROAS" | 广告查询 |
...
1. 进入 **Connectors** 设置,点击 **Add custom connector(添加自定义连接器)**。
   - Claude Desktop:点击左下角你的名字 → **Settings → Connectors → Add custom connector**。(或在对话中点击 **Customize(自定义)→ Connectors → Add custom connector**。)
   - 网页版 Claude:个人菜单 → **Settings → Connectors → Add custom connector**。
2. 填写:
...
Codex CLI 支持带 OAuth 登录的远程 streamable-HTTP MCP server。
...
或手动写入 `~/.codex/config.toml`(项目级 `.codex/config.toml` 同样可用):
...
| 4 | `get_ads_query_materials` | 将名称(如 profile、广告活动等)解析为查询所需的 ID。 |
...
> 用 Helium 10 Ads Query,针对我的美国亚马逊 profile,按天给我看最近 7 天的广告花费、ACoS 和 ROAS。
